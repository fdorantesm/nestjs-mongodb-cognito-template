...
async function testTokens() {
  return new Promise<void>((resolve, reject) => {
    console.log('🔍 Probando endpoint /auth/me con tokens de Cognito...\n');
      ...
      res.on('end', () => {
        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
          ...
          if (res.statusCode === 401) {
            console.error('\n⚠️  Error de autenticación - Verificar:');
            console.error('1. Tokens válidos y no expirados');
            console.error(
              '2. Headers correctos (x-access-token y authorization)',
            );
            console.error('3. Guard configurado correctamente');
          }
